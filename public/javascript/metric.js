...
async function getCitymapData(city, country) {
  ...
  try {
    ...
    const singleCityData = { city, cityLat, cityLng, countryCode };
...
function getCountryCode(country) {
  ...
  let data = countryCodeData.filter((data) => data.country_name == country);
  if (!data) {
    console.log(country, data);
    return;
  } else {
    let countryCode = data[0].country_code;
    return countryCode;
  }
};
